...
### 전투 — 원작 재현 "덱 순환식 카드배틀" (v1.0)
원작 플레이어의 증언을 명세 삼아 재현한 전투입니다:
- **덱을 짜고, 매 라운드 카드 1장을 내서 그 카드로 공격한다** — 카드에 ⚔공격·👟회피 능력치
- 낸 카드는 버려지고 덱이 순환한다 (소진 시 리셔플). **레벨업하면 덱 지참 한도 증가** (4+레벨)
- **민첩 높은 쪽이 선공** (마녀의 저주로 빼앗을 수 있다), 고유 스킬은 전투당 1회
- 스토리 적은 5라운드마다 회피 불가 **흑마법**을 쓴다 — 장기전은 위험하다
- 소모품(물약류)은 카드와 별개 — 라운드당 1개, 쓰면 사라진다
- 불리하면 **도주** 가능 (페널티 없음)
...
| 🔍 아리아 벨 · 천재소녀 | 테크니컬 딜러 | **완전분석** — 카드 2장 드로우 | 기본 |
| 📰 잭 하퍼 · 열혈기자 | 스피드 딜러 | **특종감각** — 3라운드 공격력 ×1.5 | 기본 |
| 🦇 비올레타 · 심야의 후원자 | 흡혈 브루저 | **갈증** — 공격 데미지 20% 흡혈 (지속) | 3장 클리어 |
| 🕸️ 마고 · 파문당한 견습 마녀 | 저주 캐스터 | **마녀의 저주** — 적 민첩 −6, 선공 탈취 (원작 마녀 스킬 재현) | 5장 클리어 |
...
- **🃏 카드 뽑기 · 상점** — 전투 카드 4등급 11종, 확률 상시 공시(60/25/12/3%), **30회 천장**, 10연차 + 소모품 상점. 재화는 인게임 골드뿐, 실과금 없음
- **⬆️ 에이스 카드 강화** — 골드로 카드를 +5까지 육성(⚔+2·👟+2%p씩). "이 카드로 싸운다"는 원작의 애착 육성 — 5장 보스 승률을 +25%p 끌어올린다
- **📚 도감·덱 편성** — 미보유 실루엣·수집 진척, 넣고 빼는 덱 빌딩 + 자동 편성
...
```bash
node tools/balance-sim.js   # 챕터·던전·PvP 리그 승률 검증 (통합)
```
...
| **v1.0** | **원작 재현: 덱 순환식 전투 전면 교체 (원작 플레이어 증언 기반), 덱 편성, 에이스 강화, 소모품·상점** |
| 백로그 | 현대 메타 리메이크 단계, 정식 일러스트, 스토리 2부, 온라인 랭킹 |
